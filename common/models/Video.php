...
class Video extends \yii\db\ActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            [['description'], 'string'],
            [['img'],'file', 'extensions'=>'jpg, jpeg, png, gif'],
    ...
    public function beforeDelete(){
        $models = VideoLang::find()->filterWhere(['parent'=>$this->id])->all();
        foreach ($models as $one) {
            $one->delete();
        }
        if(is_file(realpath(dirname(__FILE__)).'/../../'.Yii::$app->params['video_img'].$this->img)){
            unlink(realpath(dirname(__FILE__)).'/../../'.Yii::$app->params['video_img'].$this->img);
        }

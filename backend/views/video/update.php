...
{
    $lang_model = \common\models\VideoLang::find()->andFilterWhere(['parent' => $model->id, 'lang' => $language->id])->one();
    if(empty($lang_model) || $lang_model == NULL)  {
        $lang_model = new \common\models\VideoLang();
        $action = "/video-lang/create";
    }
    else
    {
        $lang_model = \common\models\VideoLang::findOne($lang_model->id);
        $action = "/video-lang/update?id=".$lang_model->id;
    }
    $lang_model->parent = $model->id;
    $lang_model->lang = $language->id;
    $langs[] = [
        'label' => $language->name,
        'content' => $this->render('//video-lang/_form',['model' => $lang_model, 'action' => $action, 'id' => $language->id]),

...
{
    $lang_model = \common\models\LibrarySubLang::find()->andFilterWhere(['parent' => $model->id, 'lang' => $language->id])->one();
    if(empty($lang_model) || $lang_model == NULL)  {
        $lang_model = new \common\models\LibrarySubLang();
        $action = "/library-sub-lang/create";
    }
    else
    {
        $lang_model = \common\models\LibrarySubLang::findOne($lang_model->id);
        $action = "/library-sub-lang/update?id=".$lang_model->id;
    }
    $lang_model->parent = $model->id;
    $lang_model->lang = $language->id;
    $langs[] = [
        'label' => $language->name,
        'content' => $this->render('//library-sub-lang/_form',['model' => $lang_model, 'action' => $action, 'id' => $language->id]),

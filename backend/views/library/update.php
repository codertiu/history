...
{
    $lang_model = \common\models\LibraryLang::find()->andFilterWhere(['parent' => $model->id, 'lang' => $language->id])->one();
    if(empty($lang_model) || $lang_model == NULL)  {
        $lang_model = new \common\models\LibraryLang();
        $action = "/library-lang/create";
    }
    else
    {
        $lang_model = \common\models\LibraryLang::findOne($lang_model->id);
        $action = "/library-lang/update?id=".$lang_model->id;
    }
    $lang_model->parent = $model->id;
    $lang_model->lang = $language->id;
    $langs[] = [
        'label' => $language->name,
        'content' => $this->render('//library-lang/_form',['model' => $lang_model, 'action' => $action, 'id' => $language->id]),

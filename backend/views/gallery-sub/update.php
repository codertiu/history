...
{
    $lang_model = \common\models\GallerySubLang::find()->andFilterWhere(['parent' => $model->id, 'lang' => $language->id])->one();
    if(empty($lang_model) || $lang_model == NULL)  {
        $lang_model = new \common\models\GallerySubLang();
        $action = "/gallery-sub-lang/create";
    }
    else
    {
        $lang_model = \common\models\GallerySubLang::findOne($lang_model->id);
        $action = "/gallery-sub-lang/update?id=".$lang_model->id;
    }
    $lang_model->parent = $model->id;
    $lang_model->lang = $language->id;
    $langs[] = [
        'label' => $language->name,
        'content' => $this->render('//gallery-sub-lang/_form',['model' => $lang_model, 'action' => $action, 'id' => $language->id]),

...
{
    $lang_model = \common\models\CategoryLang::find()->andFilterWhere(['parent' => $model->id, 'lang' => $language->id])->one();
    if(empty($lang_model) || $lang_model == NULL)  {
        $lang_model = new \common\models\CategoryLang();
        $action = "/category-lang/create";
    }
    else
    {
        $lang_model = \common\models\CategoryLang::findOne($lang_model->id);
        $action = "/category-lang/update?id=".$lang_model->id;
    }
    $lang_model->parent = $model->id;
    $lang_model->lang = $language->id;
    $langs[] = [
        'label' => $language->name,
        'content' => $this->render('//category-lang/_form',['model' => $lang_model, 'action' => $action, 'id' => $language->id]),
